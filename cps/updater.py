...
class Updater(threading.Thread):
    ...
    def do_work(self):
        try:
            ...
            self.status = 12
            log.error(
                "Possible Reason for error: update file could not be saved in temp dir"
            )
        ...
        for src_dir, __, files in os.walk(root_src_dir):
            ...
            if not os.path.exists(dst_dir):
                try:
                    ...
                    log.error(
                        "Failed creating folder: {} with error {}".format(dst_dir, e)
                    )
                if change_permissions:
                    try:
                        os.chown(
                            dst_dir, new_permissions.st_uid, new_permissions.st_gid
                        )
                    except OSError as e:
            ...
            for file_ in files:
                ...
                if os.path.exists(dst_file):
                    ...
                    try:
                        ...
                        log.error(
                            "Failed removing file: {} with error {}".format(dst_file, e)
                        )
                else:
                    if change_permissions:
                ...
                try:
                    ...
                    log.error(
                        "Failed moving file from {} to {} with error {}".format(
                            src_file, dst_dir, ex
                        )
                    )
                if change_permissions:
    ...
    def update_source(self, source, destination):
        ...
        if constants.HOME_CONFIG:
            shutil.copy(
                os.path.join(source, "cps.py"), os.path.join(source, "__main__.py")
            )
            shutil.move(
                os.path.join(source, "cps.py"), os.path.join(source, "__init__.py")
            )
    ...
    def _nightly_version_info(cls):
        if (
            is_sha1(constants.NIGHTLY_VERSION[0])
            and len(constants.NIGHTLY_VERSION[1]) > 0
        ):
            log.debug(
                "Nightly version: {}, {}".format(
                    constants.NIGHTLY_VERSION[0], constants.NIGHTLY_VERSION[1]
                )
            )
            return {
                "version": constants.NIGHTLY_VERSION[0],
                "datetime": constants.NIGHTLY_VERSION[1],
            }
        return False
            ...
            while True:
                ...
                if parent_commit["sha"] != status["current_commit_hash"]:
                    try:
                        headers = {"Accept": "application/vnd.github.v3+json"}
                        r = requests.get(
                            parent_commit["url"], headers=headers, timeout=10
                        )
                        r.raise_for_status()
                        parent_data = r.json()
                        ...
                        parent_commit_date = (
                            datetime.datetime.strptime(
                                parent_data["committer"]["date"], "%Y-%m-%dT%H:%M:%SZ"
                            )
                            - tz
                        )
    ...
    def _load_nightly_data(repository_url, commit, status):
        ...
        try:
            headers = {"Accept": "application/vnd.github.v3+json"}
            r = requests.get(
                repository_url + "/git/commits/" + commit["object"]["sha"],
                headers=headers,
                timeout=10,
            )
            r.raise_for_status()
    ...
    def _add_excluded_files(log_function):
        excluded_files = [
            ...
            os.sep + "_sqlite3.so",
            os.sep + "cps" + os.sep + ".HOMEDIR",
            os.sep + "gmail.json",
            os.sep + "exclude.txt",
            os.sep + "cps" + os.sep + "cache",
        ]
        try:
    ...
    def _nightly_available_updates(self, request_method):
        ...
        if request_method == "GET":
            ...
            status, commit = self._load_remote_data(
                repository_url + "/git/refs/heads/master"
            )
            parents = []
            if status["message"] != "":
            ...
            status["update"] = True
            status, update_data = self._load_nightly_data(
                repository_url, commit, status
            )
            ...
            try:
                ...
                new_commit_date = (
                    datetime.datetime.strptime(
                        update_data["committer"]["date"], "%Y-%m-%dT%H:%M:%SZ"
                    )
                    - tz
                )
                parents.append(
                ...
                if status["current_commit_hash"] != "":
                    parents = self._populate_parent_commits(
                        update_data, status, tz, parents
                    )
            ...
            status.update(
                {
                    ...
                    "success": True,
                    "message": _(
                        "A new update is available. Click on the button below to "
                        "update to version: %(version)s",
    ...
    def _stable_updater_parse_major_version(
        self, commit, i, parents, current_version, status
    ):
    ...
    def _stable_available_updates(self, request_method):
        ...
        if request_method == "GET":
            ...
            while i >= 0:
                if (
                    "tag_name" not in commit[i]
                    or "body" not in commit[i]
                    or "zipball_url" not in commit[i]
                ):
                    status["message"] = _(
                        "Unexpected data while reading update information"
                    )
                ...
                try:
                    ...
                    current_version[2] = (
                        int(current_version[2].replace("b", "").split(" ")[0]) - 1
                    )
                ...
                if major_version_update == current_version[0]:
                    if (
                        ...
                        and patch_version_update > current_version[2]
