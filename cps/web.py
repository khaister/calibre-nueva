...
@limiter.limit(
    "40/day", key_func=lambda: request.form.get("username", "").strip().lower()
)
@limiter.limit(
    "3/minute", key_func=lambda: request.form.get("username", "").strip().lower()
)
def login_post():
    form = request.form.to_dict()
    username = (
        form.get("username", "").strip().lower().replace("\n", "").replace("\r", "")
    )
    try:
        limiter.check()
    except RateLimitExceeded:
