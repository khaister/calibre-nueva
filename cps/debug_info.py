...
def assemble_logfiles(file_name):
    ...
    if int(__version__.split(".")[0]) < 2:
        return send_file(
            wfd, as_attachment=True, attachment_filename=os.path.basename(file_name)
        )
    else:
        return send_file(
            wfd, as_attachment=True, download_name=os.path.basename(file_name)
        )
...
def send_debug():
    ...
    file_list.extend(
        glob.glob(logger.get_accesslogfile(config.config_access_logfile) + "*")
    )
    ...
    with zipfile.ZipFile(memory_zip, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr(
            "settings.txt", json.dumps(config.to_dict(), sort_keys=True, indent=2)
        )
        zf.writestr(
            "libs.txt",
            json.dumps(collect_stats(), sort_keys=True, indent=2, cls=lazyEncoder),
        )
        for fp in file_list:
    ...
    if int(__version__.split(".")[0]) < 2:
        ...
        return send_file(
            memory_zip, as_attachment=True, download_name="Calibre-Web-debug-pack.zip"
        )
