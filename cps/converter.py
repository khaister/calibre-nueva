...
def get_calibre_version():
    return _get_command_version(
        config.config_converterpath, r"ebook-convert.*\(calibre", "--version"
    )
...
def get_unrar_version():
    ...
    if unrar_version == "not installed":
        unrar_version = _get_command_version(
            config.config_rarfile_location, r"unrar.*\d", "-V"
        )
