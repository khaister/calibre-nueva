...
def feed_letter_books(book_id):
    ...
    letter = (
        true() if book_id == "00" else func.upper(db.Books.sort).startswith(book_id)
    )
...
def feed_authorindex():
    if not current_user.check_visibility(constants.SIDEBAR_AUTHOR):
        abort(404)
    return render_element_index(
        db.Authors.sort, db.books_authors_link, "opds.feed_letter_author"
    )
...
def feed_categoryindex():
    if not current_user.check_visibility(constants.SIDEBAR_CATEGORY):
        abort(404)
    return render_element_index(
        db.Tags.name, db.books_tags_link, "opds.feed_letter_category"
    )
...
def feed_seriesindex():
    if not current_user.check_visibility(constants.SIDEBAR_SERIES):
        abort(404)
    return render_element_index(
        db.Series.sort, db.books_series_link, "opds.feed_letter_series"
    )
    ...
    if current_user.is_anonymous:
        ...
        shelf = (
            ...
            .filter(
                or_(
                    and_(
                        ub.Shelf.user_id == int(current_user.id), ub.Shelf.id == book_id
                    ),
...
def feed_read_books():
    if not (
        current_user.check_visibility(constants.SIDEBAR_READ_AND_UNREAD)
...
def render_element_index(database_column, linked_table, folder):
    ...
    return render_xml_template(
        "feed.xml", letterelements=elements, folder=folder, pagination=pagination
    )
