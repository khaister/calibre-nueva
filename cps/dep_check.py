            ...
            with open(req_path, "r") as f:
                for line in f:
                    if (
                        ...
                        try:
                            if getattr(sys, "frozen", False):
                                dep_version = exe_deps[
                                    res.group(1).lower().replace("_", "-")
                                ]
                            else:
                                if importlib:
                                    dep_version = version(res.group(1))
                                else:
                                    dep_version = pkg_resources.get_distribution(
                                        res.group(1)
                                    ).version
...
def dependency_check(optional=False):
    ...
    for dep in deps:
        try:
            dep_version_int = [
                int(x) if x.isnumeric() else 0 for x in dep[0].split(".")
            ]
            ...
            d.append({"name": dep[1], "target": "available", "found": "Not available"})
            continue

        if dep[2].strip() == "==":
            if dep_version_int != low_check:
                d.append({"name": dep[1], "found": dep[0], "target": dep[2] + dep[3]})
                continue
        elif dep[2].strip() == ">=":
            if dep_version_int < low_check:
                d.append({"name": dep[1], "found": dep[0], "target": dep[2] + dep[3]})
        ...
        if dep[4] and dep[5]:
            if dep[4].strip() == "<":
                if dep_version_int >= high_check:
                    d.append(
                        {"name": dep[1], "found": dep[0], "target": dep[4] + dep[5]}
                    )
                    continue
            elif dep[4].strip() == "<=":
                if dep_version_int > high_check:
