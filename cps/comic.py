...
try:
    ...
    try:
        ...
        log.debug(
            "Cannot import rarfile, extracting cover files from rar files will not work: %s",
            e,
        )
        use_rarfile = False
    try:
        ...
        log.debug(
            "Cannot import py7zr, extracting cover files from CB7 files will not work: %s",
            e,
        )
        use_7zip = False
...
def _extract_cover(tmp_file_name, original_file_extension, rar_executable):
    ...
    if use_comic_meta:
        ...
        name_list = (
            archive.getPageNameList
            if hasattr(archive, "getPageNameList")
            else archive.get_page_name_list
        )
...
def get_comic_info(
    tmp_file_path, original_file_name, original_file_extension, rar_executable
):
    if use_comic_meta:
        try:
        ...
        if seems_archive():
            has_metadata = (
                archive.hasMetadata
                if hasattr(archive, "hasMetadata")
                else archive.has_metadata
            )
            ...
            read_metadata = (
                ...
                if hasattr(archive, "readMetadata")
                else archive.read_metadata
            )
            ...
            return BookMeta(
                ...
                cover=_extract_cover(
                    tmp_file_path, original_file_extension, rar_executable
                ),
