...
class mySimpleLDap(LDAP):
    ...
    def initialize(self):
        ...
        try:
            log_level = (
                2 if current_app.config["LDAP_LOGLEVEL"] == logger.logging.DEBUG else 0
            )
            conn = pyLDAP.initialize(
                "{0}://{1}:{2}".format(
                ...
                trace_file=LDAPLogger(),
            )
            conn.set_option(
                pyLDAP.OPT_NETWORK_TIMEOUT, current_app.config["LDAP_TIMEOUT"]
            )
