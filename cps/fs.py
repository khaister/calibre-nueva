...
class FileSystem:
    ...
    def get_cache_dir(self, cache_type=None):
        if not isdir(self._cache_dir):
            try:
                ...
                self.log.info(
                    f"Failed to create path {self._cache_dir} (Permission denied)."
                )
                raise
    ...
    def get_cache_file_path(self, filename, cache_type=None):
        return (
            join(self.get_cache_file_dir(filename, cache_type), filename)
            if filename
            else None
        )
    ...
    def delete_cache_dir(self, cache_type=None):
        if not cache_type and isdir(self._cache_dir):
            try:
                ...
                self.log.info(
                    f"Failed to delete path {self._cache_dir} (Permission denied)."
                )
                raise
