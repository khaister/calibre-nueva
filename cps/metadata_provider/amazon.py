...
class Amazon(Metadata):
    ...
    def search(
        ...
        def inner(link, index) -> [dict, int]:
            with self.session as session:
                ...
                try:
                    ...
                    try:
                        match.description = (
                            "\n".join(
                                soup2.find(
                                    "div",
                                    attrs={"data-feature-name": "bookDescription"},
                                ).stripped_strings
                            )
                            .replace("\xa0", " ")[:-9]
                            .strip()
                    ...
                    try:
                        match.rating = int(
                            soup2.find("span", class_="a-icon-alt")
                            .text.split(" ")[0]
                            .split(".")[0]
                    ...
                    try:
                        match.cover = soup2.find(
                            "img", attrs={"class": "a-dynamic-image frontImage"}
                        )["src"]
                    except (AttributeError, TypeError):
                        match.cover = ""
                    return match, index
        ...
        if self.active:
            ...
            links_list = [
                next(filter(lambda i: "digital-text" in i["href"], x.findAll("a")))[
                    "href"
                ]
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                fut = {
                    executor.submit(inner, link, index)
        ...
        return [
            x[0] for x in sorted(result, key=itemgetter(1))
