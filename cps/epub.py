...
def get_epub_layout(book, book_data):
    ...
    try:
        ...
        layout = p.xpath(
            'pkg:meta[@property="rendition:layout"]/text()', namespaces=default_ns
        )
    except (etree.XMLSyntaxError, KeyError, IndexError, OSError) as e:
        log.error(
            "Could not parse epub metadata of book {} during kobo sync: {}".format(
                book.id, e
            )
        )
        layout = []
...
def parse_epub_cover(ns, tree, epub_zip, cover_path, tmp_file_path):
    ...
    meta_cover = tree.xpath(
        "/pkg:package/pkg:metadata/pkg:meta[@name='cover']/@content", namespaces=ns
    )
    if len(meta_cover) > 0:
        cover_section = tree.xpath(
            "/pkg:package/pkg:manifest/pkg:item[@id='" + meta_cover[0] + "']/@href",
            namespaces=ns,
        )
        if not cover_section:
            cover_section = tree.xpath(
                "/pkg:package/pkg:manifest/pkg:item[@properties='"
                + meta_cover[0]
                + "']/@href",
                namespaces=ns,
            )
        ...
        cover_section = tree.xpath(
            "/pkg:package/pkg:guide/pkg:reference/@href", namespaces=ns
        )
    ...
    for cs in cover_section:
        if cs.endswith(".xhtml") or cs.endswith(".html"):
            ...
            if not len(img_src):
                img_src = markup_tree.xpath(
                    "//attribute::*[contains(local-name(), 'href')]"
                )
            if len(img_src):
                # img_src maybe start with "../"" so fullpath join then relpath to cwd
                filename = os.path.relpath(
                    os.path.join(
                        os.path.dirname(os.path.join(cover_path, cover_section[0])),
                        img_src[0],
                    )
                )
