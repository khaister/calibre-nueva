        ...
        if use_IM and self.stat != STAT_CANCELLED and self.stat != STAT_ENDED:
            ...
            for i, book in enumerate(books_with_covers):
                ...
                if generated > 0:
                    ...
                    self.message = N_(
                        "Generated %(count)s cover thumbnails", count=total_generated
                    )
    ...
    def generate_book_thumbnail(self, book, thumbnail):
        if book and thumbnail:
            if config.config_use_google_drive:
                    ...
                    with Image(file=stream) as img:
                        ...
                        if img.height > height:
                            width = get_resize_width(
                                thumbnail.resolution, img.width, img.height
                            )
                ...
                book_cover_filepath = os.path.join(
                    config.get_book_path(), book.path, "cover.jpg"
                )
...
class TaskGenerateSeriesThumbnails(CalibreTask):
    ...
    def run(self, worker_thread):
        if (
            self.calibre_db.session
            and use_IM
            and self.stat != STAT_CANCELLED
            and self.stat != STAT_ENDED
        ):
            ...
            for i, series in enumerate(all_series):
                ...
                for thumbnail in series_thumbnails:
                    if any(
                        book.last_modified > thumbnail.generated_at
                ...
                if generated > 0:
                    total_generated += generated
                    self.message = N_("Generated {0} series thumbnails").format(
                        total_generated
                    )
    ...
    def generate_series_thumbnail(self, series_books, thumbnail):
        ...
        books = sorted(series_books, key=lambda b: float(b.series_index), reverse=True)[
            :4
        ]
...
class TaskClearCoverThumbnailCache(CalibreTask):
    ...
    def delete_thumbnail(self, thumbnail):
        try:
            ...
            self.app_db_session.query(ub.Thumbnail).filter(
                ub.Thumbnail.type == constants.THUMBNAIL_TYPE_COVER
            ).filter(ub.Thumbnail.entity_id == thumbnail.entity_id).delete()
            self.app_db_session.commit()
        except Exception as ex:
