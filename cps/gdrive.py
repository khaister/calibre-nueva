...
def watch_gdrive():
    if not config.config_google_drive_watch_changes_response:
        ...
        address = (
            filedata["web"]["redirect_uris"][0]
            .rstrip("/")
            .replace("/gdrive/callback", "/gdrive/watch/callback")
        )
        notification_id = str(uuid4())
        try:
            result = gdriveutils.watchChange(
                gdriveutils.Gdrive.Instance().drive,
                notification_id,
                "web_hook",
                address,
            ...
            if reason["reason"] == "push.webhookUrlUnauthorized":
                flash(
                    _(
                        "Callback domain is not verified, "
                        "please follow steps to verify domain in google developer console"
                    ),
                    category="error",
                )
            else:
...
try:
    ...
    def on_received_watch_confirmation():
        if not config.config_google_drive_watch_changes_response:
            return ""
        if (
            request.headers.get("X-Goog-Channel-Token") != gdrive_watch_callback_token
        ...
        try:
            ...
            response = gdriveutils.getChangeById(
                gdriveutils.Gdrive.Instance().drive, j["id"]
            )
            log.debug("%r", response)
            if response:
                dbpath = os.path.join(config.config_calibre_dir, "metadata.db").encode()
                if (
                    not response["deleted"]
                    ...
                    copyfile(
                        dbpath,
                        os.path.join(
                            tmp_dir, "metadata.db_" + str(current_milli_time())
                        ),
                    )
                    log.info("Backing up existing and downloading updated metadata.db")
                    gdriveutils.downloadFile(
                        None, "metadata.db", os.path.join(tmp_dir, "tmp_metadata.db")
                    )
