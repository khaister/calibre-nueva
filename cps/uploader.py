...
try:
    ...
    log.debug(
        "Cannot import Image, generating pdf covers for pdf uploads will not work: %s",
        e,
    )
...
try:
    ...
    log.debug(
        "PyPDF is recommended for best performance in metadata extracting from pdf files: %s",
        ex,
    )
    try:
        ...
        log.debug(
            "PyPdf2 is also possible for metadata extracting from pdf files, but not recommended anymore"
        )
        try:
            ...
            log.debug(
                "Cannot import PyPDF3/PyPDF2, extracting pdf metadata will not work: %s / %s",
                e,
            )
            use_pdf_meta = False
...
def process(tmp_file_path, original_file_name, original_file_extension, rar_executable):
    ...
    try:
        if ".PDF" == extension_upper:
            ...
            meta = epub.get_epub_info(
                tmp_file_path, original_file_name, original_file_extension
            )
        elif ".FB2" == extension_upper and use_fb2_meta is True:
            meta = fb2.get_fb2_info(tmp_file_path, original_file_extension)
        elif extension_upper in [".CBZ", ".CBT", ".CBR", ".CB7"]:
            meta = comic.get_comic_info(
                tmp_file_path,
                original_file_name,
...
def pdf_meta(tmp_file_path, original_file_name, original_file_extension):
    ...
    if doc_info:
        if author == "":
            author = (
                " & ".join(split_authors([doc_info.author]))
                if doc_info.author
                else "Unknown"
            )
        if title == "":
